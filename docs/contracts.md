...
5. Do not mutate old database JSON, backfill invented facts, or globally route legacy code through a new writer. B03 integrates Dataset 2.0 across scoped intake/storage reads, adapters, report/replay and frontend consumers. Legacy source-bearing CSV requests still produce Dataset 1.0. Other new versions remain contract-only until their producers and consumers are integrated. See [intake compatibility](intake.md).
...
C07 now produces Failure 2.0 through the trusted actor-aware outcome service and persists EvaluationProtocol 1.0 for predeclared comparisons. Runtime/HTTP/browser readers support both, while the manual Failure 1.0 writer and legacy reader remain intact. See [unsuccessful outcomes](unsuccessful-outcomes.md) for source-job verification, protocol fingerprints, absent metrics, runtime snapshots and C12 exposure limits. Earlier descriptions of these two shapes as contract-only are superseded by this integration.

C11 adds [verified references and ClaimSet production](evidence-references.md), including immutable named source anchors and scoped graph resolution. C12 adds [sealed comparisons and durable exposure](evaluation-exposure.md), `EvaluationView`/`EvaluationStatusView` read projections, and Capabilities **1.1**. These integrations supersede the earlier contract-only/pending descriptions. Legacy artifact payloads remain unchanged. Apply metadata migration 0005; regenerate strict browser contracts alongside the server.
