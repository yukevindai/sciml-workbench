...
- Workers bound runtime. Reports freeze metadata and a snapshot digest at submission under the publication barrier; archive assembly uses that capture outside locks. Project exports require settled scientific work; the internal selected-run interface checks only included producers. Persisted run authorization still requires B11/E03. See [report capture](report-capture.md).
