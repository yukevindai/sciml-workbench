...
Clients uncertain about acceptance must retain and resend the exact key and payload, or read existing jobs; generating a new key deliberately requests another operation. The browser does this for audit, split, benchmark, failure and report submissions (A09): the key and exact body are kept in local storage until the server accepts or definitively refuses them, and are only resent by an explicit researcher action. See the [A09 handoff](tickets/A09.md).
