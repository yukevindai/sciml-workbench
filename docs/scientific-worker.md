...
A timeout can occur after an upstream import committed but before its response was recorded. Do not infer that the external write failed or blindly retry it under a new identity. B07/D04 provide the durable import journal and reconciliation. B05/B08 retain ownership of complete scoped lineage and publication semantics, B06 of a request-time report snapshot, D11 of agent scheduling and D12 of run cancellation. Reports currently copy metadata at execution preparation, not at submission, and do not implement the future snapshot barrier.
