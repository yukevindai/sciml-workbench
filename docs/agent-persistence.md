...
`GET /api/v1/projects/{p}/execution-policy` (A11) is a read-only `ExecutionPolicySummary` for the request workspace. It returns the current server/project intersection, the stored project-policy row revision that run admission compares, and whether agent admission is currently available (with its configuration reason when not). Material and artifact grants are listed only for records owned by that project; provider model identities are omitted. A project without both policies, or not named by the intersection, returns `policy: null`. There is still no policy-write route.
