...
async function proxy(request: NextRequest, { params }: { params: Promise<{ path: string[] }> }) {
  ...
  if (request.method !== 'GET' && request.headers.get('origin') !== (process.env.WB_PUBLIC_ORIGIN || 'http://localhost:3000'))
    return Response.json({ error: 'Cross-origin request rejected' }, { status: 403 });
  const token = process.env.WB_API_TOKEN;
  if (!token) return Response.json({ error: 'Server API connection is not configured' }, { status: 503 });
  const headers = new Headers({ Authorization: `Bearer ${token}` });
  for (const name of ['content-type', 'x-filename', 'x-source', 'x-title', 'idempotency-key']) {
  ...
  try {
    ...
    const response = await fetch(`${process.env.WB_API_URL || 'http://localhost:8000'}/api/v1/${path.join('/')}${request.nextUrl.search}`, {
      method: request.method, headers, body: body as BodyInit | undefined, cache: 'no-store', signal: AbortSignal.timeout(60000)
    });
